...
func (s *FileTokenStore) List(ctx context.Context) ([]*cliproxyauth.Auth, error) {
	...
	for _, file := range files {
		...
		auths, errReadAuths := s.readAuthFiles(file.Path, dir, file.ID)
		if errReadAuths != nil {
			continue
		}
		if len(auths) > 0 {
...
func (s *FileTokenStore) readAuthFiles(path, baseDir string, explicitIDs ...string) ([]*cliproxyauth.Auth, error) {
	...
	fileID := s.idFor(path, baseDir)
	if len(explicitIDs) > 0 && strings.TrimSpace(explicitIDs[0]) != "" {
		fileID = strings.TrimSpace(explicitIDs[0])
	}
