...
func TestForwardResponsesStreamClientCanceledWritesNoErrorChunk(t *testing.T) {
	gin.SetMode(gin.TestMode)
	base := handlers.NewBaseAPIHandlers(&sdkconfig.SDKConfig{}, nil)
	h := NewOpenAIResponsesAPIHandler(base)
	...
	errs <- &interfaces.ErrorMessage{
		StatusCode: handlers.StatusClientClosedRequest,
		Error:      errors.New(`Post "https://example.test/responses": context canceled`),
	}
	close(errs)

	h.forwardResponsesStream(c, flusher, func(error) {}, data, errs, nil)
	if status := c.Writer.Status(); status != handlers.StatusClientClosedRequest {
		t.Fatalf("status = %d, want %d", status, handlers.StatusClientClosedRequest)
	}
