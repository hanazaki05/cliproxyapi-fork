...
func (r *UsageReporter) buildRecord(detail usage.Detail, failed bool, failures ...usage.Failure) usage.Record {
	...
	if r == nil {
		return usage.Record{Detail: detail, Failed: failed, Outcome: usageOutcome(failed, fail), Fail: fail, Generate: usage.GenerateFlag(true)}
	}
...
func (r *UsageReporter) buildRecordForModel(model string, detail usage.Detail, failed bool, fail usage.Failure) usage.Record {
	if r == nil {
		return usage.Record{Model: model, Detail: detail, Failed: failed, Outcome: usageOutcome(failed, fail), Fail: fail, Generate: usage.GenerateFlag(true)}
	}
	return usage.Record{
		Provider:            r.provider,
