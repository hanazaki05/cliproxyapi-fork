...
func (e *CodexExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	httpReq, upstreamBody, identityState, err := e.cacheHelper(ctx, opts, from, url, auth, req, originalPayloadSource, body, opts.Headers)
	if err != nil {
		return resp, err
	}
...
func (e *CodexExecutor) cacheHelper(ctx context.Context, opts cliproxyexecutor.Options, from sdktranslator.Format, url string, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, userPayload []byte, rawJSON []byte, headerSets ...http.Header) (*http.Request, []byte, codexIdentityConfuseState, error) {
	...
	var requestState codexRequestState
	if codexRequestAlignEnabled(e.cfg) {
		requestState, rawJSON = prepareCodexRequestState(ctx, opts, from, req, rawJSON)
		if requestState.ConversationID != "" {
			cache.ID = requestState.ConversationID
		}
	}
	...
	if cache.ID != "" {
		setCodexSessionHeaderCasePreserved(httpReq.Header, "session-id", cache.ID)
	}
	if codexRequestAlignEnabled(e.cfg) {
		applyCodexRequestStateHeaders(httpReq.Header, requestState)
	}
...
func applyCodexHeadersFromSources(r *http.Request, auth *cliproxyauth.Auth, token string, stream bool, cfg *config.Config, ginHeaders http.Header) {
	...
	if codexRequestAlignEnabled(cfg) {
		...
		ensureCodexConversationTurnHeaders(r.Header, ginHeaders)
	} else if strings.Contains(r.Header.Get("User-Agent"), "Mac OS") {
		ensureCodexWebsocketSessionHeader(r.Header, ginHeaders, uuid.NewString())
	}
