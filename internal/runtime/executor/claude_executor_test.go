...
func TestApplyClaudeHeaders_AppliesConfiguredBetaRulesForMatchingProviderAndModel(t *testing.T) {
	resetClaudeDeviceProfileCache()

	cfg := &config.Config{
		ClaudeHeaderDefaults: config.ClaudeHeaderDefaults{
			BetaRules: []config.ClaudeBetaRule{
				{
					Provider: "custom",
					Model:    "deepseek-v4-*",
					Remove:   []string{"fast-mode-2026-02-01"},
					Add:      []string{"custom-beta-2026-04-26"},
				},
			},
		},
	}
	auth := &cliproxyauth.Auth{
		ID:       "auth-custom-beta-rule",
		Provider: "custom",
		Attributes: map[string]string{
			"api_key":      "key-custom-beta-rule",
			"provider_key": "custom",
		},
	}

	req := newClaudeHeaderTestRequest(t, http.Header{})
	applyClaudeHeaders(req, auth, "key-custom-beta-rule", false, nil, "deepseek-v4-flash", cfg)

	got := req.Header.Get("Anthropic-Beta")
	if strings.Contains(got, "fast-mode-2026-02-01") {
		t.Fatalf("Anthropic-Beta = %q, expected fast-mode beta to be removed", got)
	}
	for _, beta := range []string{"custom-beta-2026-04-26", "oauth-2025-04-20", "interleaved-thinking-2025-05-14"} {
		if !strings.Contains(got, beta) {
			t.Fatalf("Anthropic-Beta = %q, want to contain %q", got, beta)
		}
	}
}

func TestApplyClaudeHeaders_AppliesConfiguredBetaRulesForMatchingEndpointHost(t *testing.T) {
	resetClaudeDeviceProfileCache()
	...
	cfg := &config.Config{
		ClaudeHeaderDefaults: config.ClaudeHeaderDefaults{
			BetaRules: []config.ClaudeBetaRule{
				{
					EndpointHosts: []string{"zenmux.ai", "example.com"},
					Model:         "deepseek/*",
					Remove:        []string{"fast-mode-2026-02-01"},
				},
			},
	...
	auth := &cliproxyauth.Auth{
		ID:       "auth-endpoint-host-beta-rule",
		Provider: "claude",
		Attributes: map[string]string{
			"api_key": "key-endpoint-host-beta-rule",
		},
	}

	req := newClaudeHeaderTestRequest(t, http.Header{})
	req.URL.Scheme = "https"
	req.URL.Host = "zenmux.ai"
	applyClaudeHeaders(req, auth, "key-endpoint-host-beta-rule", false, nil, "deepseek/deepseek-v4-flash-free", cfg)
...
func TestApplyClaudeHeaders_SkipsConfiguredBetaRulesForNonMatchingEndpointHost(t *testing.T) {
	resetClaudeDeviceProfileCache()
	...
	cfg := &config.Config{
		ClaudeHeaderDefaults: config.ClaudeHeaderDefaults{
			BetaRules: []config.ClaudeBetaRule{
				{
					EndpointHosts: []string{"zenmux.ai"},
					Model:         "deepseek/*",
					Remove:        []string{"fast-mode-2026-02-01"},
				},
			},
	...
	auth := &cliproxyauth.Auth{
		ID:       "auth-endpoint-host-beta-rule-miss",
		Provider: "claude",
		Attributes: map[string]string{
			"api_key": "key-endpoint-host-beta-rule-miss",
		},
	}
	...
	applyClaudeHeaders(req, auth, "key-endpoint-host-beta-rule-miss", false, nil, "deepseek/deepseek-v4-flash-free", cfg)

	got := req.Header.Get("Anthropic-Beta")
	if !strings.Contains(got, "fast-mode-2026-02-01") {
		t.Fatalf("Anthropic-Beta = %q, expected fast-mode beta to remain", got)
	}
}

func TestApplyClaudeHeaders_AppliesConfiguredBetaRulesWhenProviderAndEndpointHostBothMatch(t *testing.T) {
	resetClaudeDeviceProfileCache()
	...
	auth := &cliproxyauth.Auth{
		ID:       "auth-provider-endpoint-host-beta-rule",
		Provider: "claude",
		Attributes: map[string]string{
			"api_key": "key-provider-endpoint-host-beta-rule",
		},
	}
	...
	req.URL.Scheme = "https"
	req.URL.Host = "zenmux.ai"
	applyClaudeHeaders(req, auth, "key-provider-endpoint-host-beta-rule", false, nil, "deepseek/deepseek-v4-flash-free", cfg)
...
func TestApplyClaudeHeaders_SkipsConfiguredBetaRulesWhenProviderMatchesButEndpointHostMisses(t *testing.T) {
	resetClaudeDeviceProfileCache()
	...
	auth := &cliproxyauth.Auth{
		ID:       "auth-provider-endpoint-host-beta-rule-host-miss",
		Provider: "claude",
		Attributes: map[string]string{
			"api_key": "key-provider-endpoint-host-beta-rule-host-miss",
		},
	}
	...
	req.URL.Scheme = "https"
	req.URL.Host = "api.anthropic.com"
	applyClaudeHeaders(req, auth, "key-provider-endpoint-host-beta-rule-host-miss", false, nil, "deepseek/deepseek-v4-flash-free", cfg)
...
func TestApplyClaudeHeaders_SkipsConfiguredBetaRulesWhenEndpointHostMatchesButProviderMisses(t *testing.T) {
	resetClaudeDeviceProfileCache()
	...
	auth := &cliproxyauth.Auth{
		ID:       "auth-provider-endpoint-host-beta-rule-provider-miss",
		Provider: "claude",
		Attributes: map[string]string{
			"api_key": "key-provider-endpoint-host-beta-rule-provider-miss",
		},
	}
	...
	req.URL.Scheme = "https"
	req.URL.Host = "zenmux.ai"
	applyClaudeHeaders(req, auth, "key-provider-endpoint-host-beta-rule-provider-miss", false, nil, "deepseek/deepseek-v4-flash-free", cfg)
...
func TestApplyClaudeHeaders_ClientAnthropicBetaBypassesConfiguredBetaRules(t *testing.T) {
	resetClaudeDeviceProfileCache()
	...
	auth := &cliproxyauth.Auth{
		ID:       "auth-custom-beta-client-override",
		Provider: "custom",
		Attributes: map[string]string{
			"api_key":      "key-custom-beta-client-override",
			"provider_key": "custom",
		},
	}

	req := newClaudeHeaderTestRequest(t, http.Header{
		"Anthropic-Beta": []string{"client-beta-2026-04-26"},
	})
	applyClaudeHeaders(req, auth, "key-custom-beta-client-override", false, nil, "deepseek-v4-flash", cfg)

	got := req.Header.Get("Anthropic-Beta")
	if !strings.Contains(got, "client-beta-2026-04-26") {
		t.Fatalf("Anthropic-Beta = %q, want to contain %q", got, "client-beta-2026-04-26")
	}
	if strings.Contains(got, "custom-beta-2026-04-26") {
		t.Fatalf("Anthropic-Beta = %q, expected configured beta rule to be bypassed by client header", got)
	}
}
