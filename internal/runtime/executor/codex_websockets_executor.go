...
func (e *CodexWebsocketsExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	body, wsHeaders, errPromptCache := applyCodexPromptCacheHeadersWithContext(ctx, opts, from, req, body, e.cfg, opts.Headers)
...
func mapCodexWebsocketReadError(err error) error {
	...
	return statusErr{code: http.StatusRequestTimeout, msg: "stream error: " + err.Error()}
}
...
func applyCodexPromptCacheHeaders(from sdktranslator.Format, req cliproxyexecutor.Request, rawJSON []byte) ([]byte, http.Header) {
	body, headers, _ := applyCodexPromptCacheHeadersWithContext(context.Background(), cliproxyexecutor.Options{}, from, req, rawJSON, nil)
	return body, headers
}

func applyCodexPromptCacheHeadersWithContext(ctx context.Context, opts cliproxyexecutor.Options, from sdktranslator.Format, req cliproxyexecutor.Request, rawJSON []byte, cfg *config.Config, headerSets ...http.Header) ([]byte, http.Header, error) {
	headers := http.Header{}
	if len(rawJSON) == 0 {
	...
	if codexRequestAlignEnabled(cfg) {
		requestState, updatedJSON := prepareCodexRequestState(ctx, opts, from, req, rawJSON)
		applyCodexRequestStateHeaders(headers, requestState)
		return updatedJSON, headers, nil
	}
...
func applyCodexWebsocketHeaders(ctx context.Context, headers http.Header, auth *cliproxyauth.Auth, token string, cfg *config.Config) http.Header {
	...
	sessionFallback := ""
	if codexRequestAlignEnabled(cfg) {
		sessionFallback = strings.TrimSpace(headerValueCaseInsensitive(headers, "X-Client-Request-Id"))
		if sessionFallback == "" {
			sessionFallback = strings.TrimSpace(headerValueCaseInsensitive(ginHeaders, "X-Client-Request-Id"))
		}
	}
	if sessionFallback == "" && strings.Contains(headers.Get("User-Agent"), "Mac OS") {
		sessionFallback = uuid.NewString()
	}
	ensureCodexWebsocketSessionHeader(headers, ginHeaders, sessionFallback)
	if codexRequestAlignEnabled(cfg) && sessionFallback != "" {
		setCodexSessionHeaderCasePreserved(headers, "session-id", sessionFallback)
	}
